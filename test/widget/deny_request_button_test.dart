...
class TestAppCubit extends AppCubit {
  ...
  Future<void> denyRequest({
    ...
    lastDenied = message;
    // keep state as waiting; just verify call
  }
}

void main() {
  testWidgets('DenyRequestButton triggers cubit.denyRequest', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: BlocProvider<AppCubit>.value(
          ...
          child: Scaffold(
            body: Center(
              child: DenyRequestButton(
                message: msg,
                key: const Key('DenyBtn'),
              ),
            ),
          ),
        ),
    ...
    await tester.tap(find.byKey(const Key('DenyBtn')));
    await tester.pumpAndSettle();

    expect(cubit.lastDenied?.messageId, 'm100');
  });
}
